<template>
    <div class="member-recharge-record">
        <van-nav-bar title="会员充值记录" left-text="返回" left-arrow @click-left="onClickLeft"></van-nav-bar>
            ...
            <van-list v-model="loading" :finished="finished" @load="onLoad" finished-text="没有更多了">
                <van-cell v-for="(item,index) in recordList" :key="index" class="panel">
                    <div slot="title" class="panel-header">
                        <div class="left">
                            <van-icon name="vip-card" color="#f5ca3a" style="font-size: 21px; top: 5px;"/>
                            <span>会员{{item.rechargePackageDesc}}</span>
                        </div>
                        <div class="right">￥{{item.rechargeAmount}}</div>
                    </div>
                    <div slot="label" class="panel-content">
                        <p>会员生效时间：{{item.validityStartDate}}</p>
                        <p>会员到期时间：{{item.validityEndDate}}</p>
    ...
    export default {
        ...
        methods: {
            initRecordList() {
                ...
                this.$api.getMemberRechargeRecordList(reqInfo).then(res => {
